...
$("#ardoraMain").append("<div id='"+imaN+"' class='photoMG'><img src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/imaMG/" + fileIma[num-1] + "'/></div>");
if ($.trim(itemHelp[num - 1])) {$("#"+imaN).append("<div class='titleIma'><p>"+itemHelp[num - 1]+"</p></div>");}
$("#"+imaN).append("<div  id='"+titN+"' class='bClose' title='Claude'>E</div>");
$("#"+imaN).append("<div  id='"+nextN+"' class='bNext' title='Imago posterior'>C</div>");
$("#"+imaN).append("<div  id='"+prevN+"' class='bPrev' title='Imago prior'>A</div>");
if (($.trim(fileMP3[num - 1])) || ($.trim(fileOgg[num - 1]))) {audioCreate(num);}
if ($.trim(conType[num-1])){showCon(num)}
fullS(num);
if ($.trim(txt[num-1])){
$("#" + imaN).append("<div  id='" + upN + "' class='bUp' title='Sententiae'>I</div>");
...
$("#"+imaN).append("<div  id='"+txtN+"' class='bTxt' title='Res'>N</div>");
$("#" + txtN).click(function () {if ($("#"+objN).length){$("#"+objN).remove();
$("#"+imaN).append("<img src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/imaMG/" + fileIma[num-1] + "'/>");
}else{
$("#"+imaN+ " img").remove();
if (conType[num-1]=="0"){
...
$("#ardoraMain").append("<div id='imaFull' class='photoMG'><img src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/imaFull/f_" + fileIma[num-1].substring(2) + "'/></div>");
cssColors();
...
$("#imaFull").append("<img src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/imaFull/f_"+fileIma[num-1].substring(2)+"'/>");
...
var sourceLine="";
$("#imaFull").append("<div  id='musF' class='bMus' title='Audi'>K</div>");
$("#musF").click(function () {if ($("#playF").length){ $("#playF").remove();}else{
if ($.trim(fileMP3[num-1])){sourceLine=sourceLine+"<source src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/"+fileMP3[num-1]+"' type='audio/mp3' />";}
if ($.trim(fileOgg[num-1])) {sourceLine=sourceLine+"<source src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/"+fileOgg[num-1]+"' type='audio/ogg' />";}
$("#imaFull").append("<div  id='playF' class='playerAudio'><audio controls='controls'>"+sourceLine+"</audio></div>");}});}
if ($.trim(itemHelp[num - 1])) {$("#imaFull").append("<div class='titleIma'><p>" + itemHelp[num - 1] + "</p></div>");}
if ($.trim(txt[num - 1])) {$("#imaFull").append("<div  id='upF' class='bUp' title='Sententiae'>I</div>");
...
$("#"+imaN).append("<div  id='"+musN+"' class='bMus' title='Audi'>K</div>");
$("#" + musN).click(function () {
if ($("#"+playN).length){ $("#" + playN).remove();}
else{if ($.trim(fileMP3[num-1])) {
sourceLine=sourceLine+"<source src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/"+fileMP3[num-1]+"' type='audio/mp3' />";}
if ($.trim(fileOgg[num-1])) {
sourceLine=sourceLine+"<source src='02.04.00.Galeria_Imaxes.Vocabu_resources/media/"+fileOgg[num-1]+"' type='audio/ogg'/>";}
...
contexto.font="13px fActi";var indexHelp=parseInt($(item).attr("id").substring(4));var metricsW = contexto.measureText(itemHelp[indexHelp-1]).width+20;
var metricsH = 20+13+10;$("#ardoraHelpCanvas").attr({"width": metricsW,"height": metricsH});var topHelp=Math.round($(item).position().top)-metricsH;
...
contexto.font="13px fActi";contexto.shadowBlur = 0;contexto.shadowOffsetX = 0;contexto.shadowOffsetY = 0;
contexto.fillStyle=colorText;contexto.fillText(itemHelp[indexHelp-1], 5, 20);contexto.fill();}
function cssColors(){
$("body").css("background-color",colorBack);$(".photoContainer").css("background-color",colorSele);
