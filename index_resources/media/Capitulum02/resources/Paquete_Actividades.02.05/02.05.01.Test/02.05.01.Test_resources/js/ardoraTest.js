...
function paintQuestion(){randomSort();$("#imaForTag").remove();document.getElementById("ardoraTag").innerHTML=tags[indexTag];
...
if (ima[indexTag].length>0){$("#ardoraChecks").css("width","475");
...
}else{$("#ardoraChecks").css("width","700");}
...
contexto.font="10px " + fMenssage; var metricsW = contexto.measureText(textButtonTime).width; var x=(canvas.width / 2)-(metricsW / 2); var y=(canvas.height / 2); contexto.beginPath();
...
if (correct) {successes++;score = score + scoreInc;timeAct=timeAct+timeBon;if (successes==ans.length){showMessage("Ok");}else{indexTag++;paintQuestion();}
